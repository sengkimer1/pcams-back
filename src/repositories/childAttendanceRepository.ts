...
export class PostgresChildAttendanceRepository implements ChildAttendanceRepository {
  ...
  async create(data: Omit<ChildAttendance, "id" | "created_at" | "updated_at">): Promise<ChildAttendance> {
    ...
    const { rows } = await this.pool.query(
      ...
      [id, fullname, gender, age, family_id, attendance_date, status, camp_event_id, user_id, now, now] // ✅ Add `user_id` here
    );
  ...
  async findById(id: string): Promise<ChildAttendance | null> {
    const { rows } = await this.pool.query(
      `SELECT id, fullname, gender, age, family_id, attendance_date, status, camp_event_id,  created_at, updated_at FROM child_attendance WHERE id = $1`,
      [id]
    );
    return rows[0] || null;
  }
  ...
  async findByAttendanceDate(attendance_date: Date): Promise<ChildAttendance[]> {
    const { rows } = await this.pool.query(
      `SELECT id, fullname, gender, age, family_id, attendance_date, status, camp_event_id,  created_at, updated_at 
  ...
  //   let query = `SELECT id, fullname, gender, age, family_id, attendance_date, status, camp_event_id,  created_at, updated_at 
  //                FROM child_attendance`;
  //   const conditions: string[] = [];
  //   const values: (Date | string)[] = [];
  //   let paramIndex = 1;
