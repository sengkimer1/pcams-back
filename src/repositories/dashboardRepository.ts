...
export class PostgresDashboardRepository {
  ...
  async getTotalChildren(campId: string): Promise<number> {
    const result = await queryWithLogging(
      this.pool,
      `SELECT COUNT(id) AS count FROM children WHERE camp_id = $1`,
      [campId]
    );
  ...
  async getTotalCoordinators(campId: string): Promise<number> {
    const result = await queryWithLogging(
      ...
      `SELECT COUNT(u.id) AS count
       FROM Users u
       JOIN Camp_User cu ON cu.user_id = u.id
       JOIN Role r ON r.id = u.role_id
       WHERE cu.camp_id = $1 AND r.name = 'coordinator' AND cu.is_active = true`,
      [campId]
    );
    return Number(result.rows[0]?.count ?? 0);
  }
  ...
  async getTotalMonitors(campId: string): Promise<number> {
    const result = await queryWithLogging(
      ...
      `SELECT COUNT(u.id) AS count
       ...
       JOIN Camp_User cu ON cu.user_id = u.id
       JOIN Role r ON r.id = u.role_id
       WHERE cu.camp_id = $1 AND r.name = 'monitor' AND cu.is_active = true`,
      [campId]
    );
    return Number(result.rows[0]?.count ?? 0);
  }
